...
@author: yashp
'''
class MBFMultiRobot_TestBSM(Behavior):
	'''
	MBFMultiRobot_TestB
	'''
	...
	def __init__(self):
		super(MBFMultiRobot_TestBSM, self).__init__()
		self.name = 'MBFMultiRobot_TestB'
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('Counter',
										...
										transitions={'success': 'Mover', 'failed': 'failed', 'end': 'finished'},
			...
			OperatableStateMachine.add('Mover',
										MoveBaseFlexActionState(robot_name=self.robot_names),
